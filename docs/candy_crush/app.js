...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function checkRowForFour() {
    for (let i = 0; i < width * width; i++) {
      ...
      if (i % width > width - 4) continue; // Skip rows that can't form a four
  ...
  function checkRowForThree() {
    for (let i = 0; i < width * width; i++) {
      ...
      if (i % width > width - 3) continue; // Skip rows that can't form a three
  ...
  // Continuously check for matches and move candies
  setInterval(() => {
    checkRowForFour();
    checkColumnForFour();
    checkRowForThree();
...
function touchEnd() {
  ...
  if (Math.abs(deltaX) > Math.abs(deltaY)) {
    // Horizontal swipe
    squareIdBeingReplaced = squareIdBeingDragged + (deltaX > 0 ? 1 : -1);
  } else {
    // Vertical swipe
    squareIdBeingReplaced = squareIdBeingDragged + (deltaY > 0 ? width : -width);
  }

  if (squareIdBeingReplaced >= 0 && squareIdBeingReplaced < squares.length) {
